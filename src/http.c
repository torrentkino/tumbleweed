...
int http_resource( TCP_NODE *n, char *p_url, char *resource ) {
	...
	if( !http_urlDecode( p_url, strlen( p_url ), resource, BUF_SIZE) ) {
		info( &n->c_addr, 400, "Decoding resource failed" );
		return FALSE;
	}

	/* Minimum path requirement */
	if( resource[0] != '/' ) {
		info( &n->c_addr, 400, "Resource must start with '/'" );
		return FALSE;
	}

	/* ".." in path. Do not like. */
	if( strstr( resource, "../") != NULL ) {
		info( &n->c_addr, 400, "Double dots in resource" );
		return FALSE;
	}

	/* URL */
	if( ! str_isValidUTF8( resource) ) {
		info( &n->c_addr, 400, "Invalid UTF8 in resource" );
		return FALSE;
	}
